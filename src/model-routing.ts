...
export function parseGoalModelRoutingConfig(
  ...
  for (const [name, value] of Object.entries(input.scenarios)) {
    ...
    const modelClass = requireModelClass(
      value.modelClass,
      `${path}.scenarios.${name}.modelClass`,
    );
    const description =
...
function requireModelClass(input: unknown, path: string): string {
  ...
  if (!MODEL_CLASS_PATTERN.test(value)) {
    throw new Error(
      `Invalid goal model routing: ${path} must match ${MODEL_CLASS_PATTERN.source}`,
    );
  }
  return value;
}
...
function assertKnownKeys(
  ...
  for (const key of Object.keys(input)) {
    if (!allowedSet.has(key)) {
      if (key === "model") {
        throw new Error(
          `Invalid goal model routing: ${path}.model is unsupported; use modelClass`,
        );
      }
      throw new Error(
