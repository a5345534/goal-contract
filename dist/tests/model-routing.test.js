...
import { isCanonicalModelId, requireCanonicalModelId, parseGoalModelRoutingConfig, parseGoalModelRoutingConfigJson, CANONICAL_MODEL_ID_PATTERN, } from "../model-routing.js";
const validConfig = {
    scenarios: {
...
test("requireCanonicalModelId throws for dot format", () => {
    assert.throws(() => requireCanonicalModelId("openai.gpt-5.5", "test"), /canonical provider\/model/);
});
...
test("rejects non-object scenarios value", () => {
    assert.throws(() => parseGoalModelRoutingConfig({ scenarios: "bad" }), /scenarios/);
});
