...
test("rejects version !== 1", () => {
    assert.throws(() => parseGoalDagFileDocument({ ...minimalDag, version: 2 }), /version must be 1/);
});
test("rejects empty objective", () => {
    assert.throws(() => parseGoalDagFileDocument({ version: 1, objective: "", nodes: [{ id: "a", objective: "x" }] }), /non-empty/);
});
test("rejects empty nodes", () => {
    assert.throws(() => parseGoalDagFileDocument({ version: 1, objective: "x", nodes: [] }), /nodes/);
});
test("rejects malformed JSON string", () => {
    assert.throws(() => parseGoalDagFileContent("{bad"), /JSON/);
});
test("rejects unknown root keys", () => {
    assert.throws(() => parseGoalDagFileDocument({ ...minimalDag, trace: {} }), /unsupported field/);
});
...
//# sourceMappingURL=goal-dag-parser.test.js.map