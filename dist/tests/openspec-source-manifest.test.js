...
import { parseOpenSpecSourceManifest, parseOpenSpecSourceManifestJson, OPENSPEC_SOURCE_MANIFEST_SCHEMA_VERSION, OPENSPEC_AUTHORITATIVE_SOURCE_KINDS, } from "../openspec-source-manifest.js";
const validManifest = {
...
test("rejects schemaVersion !== \"1.0\"", () => {
    assert.throws(() => parseOpenSpecSourceManifest({ ...validManifest, schemaVersion: "2.0" }), /schemaVersion/);
});
test("rejects missing root keys", () => {
    assert.throws(() => parseOpenSpecSourceManifest({ schemaVersion: "1.0", sources: [] }), /changeName/);
    assert.throws(() => parseOpenSpecSourceManifest({ schemaVersion: "1.0", changeName: "x", sources: [] }), /generatedAt/);
});
test("rejects extra root keys", () => {
    assert.throws(() => parseOpenSpecSourceManifest({ ...validManifest, extra: true }), /unexpected keys/);
});
...
test("rejects empty sources", () => {
    assert.throws(() => parseOpenSpecSourceManifest({ ...validManifest, sources: [] }), /sources/);
});
test("rejects missing source required fields", () => {
    assert.throws(() => parseOpenSpecSourceManifest({
        ...validManifest,
        sources: [{ path: "x.md" }],
    }), /kind/);
});
...
//# sourceMappingURL=openspec-source-manifest.test.js.map