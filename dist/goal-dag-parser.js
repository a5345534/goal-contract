...
export function parseGoalDagFileDocument(input) {
    ...
    const modelRouting = input.modelRouting === undefined
        ? undefined
...
function parseRequiredEvidence(input, path) {
    ...
    for (let i = 0; i < input.length; i++) {
        ...
        if (seen.has(token)) {
            throw new Error(`Invalid goal DAG file: ${path} contains duplicate required evidence: ${JSON.stringify(token)}`);
        }
        seen.add(token);
        result.push(token);
    }
...
function validateFileModelScenarios(defaults, nodes, modelRouting) {
    const assertScenario = (scenario, path) => {
        if (!modelRouting || !(scenario in (modelRouting.scenarios ?? {}))) {
            throw new Error(`Invalid goal DAG file: ${path} references unknown model scenario ${JSON.stringify(scenario)}`);
        }
    };
    if (defaults?.modelScenario)
        assertScenario(defaults.modelScenario, "defaults.modelScenario");
    nodes.forEach((node, index) => {
        if (node.modelScenario)
            assertScenario(node.modelScenario, `nodes[${index}].modelScenario`);
    });
}
// ---------------------------------------------------------------------------
// Low-level parsers
...
//# sourceMappingURL=goal-dag-parser.js.map